...
int main(int argc, char **argv) {
    ...
    bs_gpu.resizeN(n);
    {
        ...
        for (int iter = 0; iter < benchmarkingIters; ++iter) {
            ...
            for (uint blockSize = 1; blockSize <= (n + 1) / 2; blockSize *= 2) {
                merge.exec(gpu::WorkSize(workGroupSize, global_work_size), as_gpu, bs_gpu, blockSize, n);
                // std::cout << std::endl;
    ...
    for (int i = 0; i < n; ++i) {
        // std::cout << i << " " << as[i] << " " << cpu_sorted[i] << std::endl;
